...
Lets start by remembering what is an operator... Depending who or where you ask is one thing or another, very similar but not the same, but everyone concludes that an operator is just a controller that bases its flow on CRDs or custom resources created by us instead of Kubernetes core resources like pods, services, deployments...

In this tutorial we will make an operator that will terminate pods, something like [chaos monkey]() but for pods and very very simple.

## 01 - Description.

Our Operator will apply chaos engineering at a very low level. It will delete pods based on the time (interval), quantity, and filtered by labels.
...
* client/k8s/clientset: This is a k8s client for our CRDs so it is easy to work with them agains kubernetes. This code is autogenerated by [code-generator](https://github.com/kubernetes/code-generator).
* cmd: The main app that will run our operator.
* log: Our logger.
* Makefile: Commands to automate stuff on the example (in this case autogenerate our CRD code).
...
The first thing that we need to do es desing our CRD or custom resource defition. In the case of this example the definition of the resource needs to be something that need to select pods based on labels and periodically kill them, also needs to respect a safe quantity of minimum pods running and lastly a dry run option to check if it's selecting the correct pods to kill. Something like this:
...
Let's start with our domain logic. Our domain logic is a single service called `Chaos`. This service will run N is responsible of running N `pod Killers`, this podkillers will be killing pods at regular intervals based on the filters and description described on our manifests (`ṔodTerminator` CRD).
...
Our service has this methods and are the ones that will be used by our operator.
...
For us create and update is the same. This means that we need to think as "ensure" or "sync" verbs instead of create/update.

## 05 - Operator

### Config

This operator it only has the resync period (the interval kubernetes will return us all the resources we are listening to)
...
You will notice that the lister watcher and the object is like a retriever. This is true because a CRD is a retriever but it knows how to initialize, nothing more. As you see on the lister watcher we are using the client that code-generator generated for us in previous steps.
...
After all these steps, a fully operation chaos operator is ready to destroy pods. You can check the [main](https://github.com/spotahome/kooper/blob/master/examples/pod-terminator-operator/cmd/main.go) where the operator is instantiated.
